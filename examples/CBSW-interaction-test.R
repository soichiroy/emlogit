...
w <- exp(cbind(rep(1, nrow(X)), X) %*% eta)
# w <- w /sum(w)
# 
...
w_cd <- w[cd_id == "TX-01"]
cd_pop <- cd_dat %>% filter(cd == "TX-01") %>% pull(educ)
apply(educ_mat[cd_id == "TX-01", ], 2, weighted.mean, w = w_cd) %>% 
  enframe() %>% 
  mutate(pop = cd_pop[[1]]$prop)



enframe(
  apply(educ_mat, 2, weighted.mean, w = exp(cbind(rep(1, nrow(X)), X) %*% eta))
)

cd_educ

enframe(apply(cd_race, 2, weighted.mean, w = exp(cbind(rep(1, nrow(X)), X) %*% eta)))
